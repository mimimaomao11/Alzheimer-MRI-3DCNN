...
from dataset import ADNIMultiChannelDataset, compute_norm_stats
...
def train_one_fold(
    ...
    stats_path = args.results_dir / f"norm_stats_mci3ch_fold{fold}.json"
    ...
    model = build_densenet121_3ch(num_classes=2, dropout_prob=args.dropout_prob).to(device)
    ...
    log_path = args.results_dir / f"training_log_mci3ch_fold{fold}.csv"
    ckpt_path = args.checkpoint_dir / f"best_mci3ch_fold{fold}.pth"
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if val_loss < best_val_loss - args.min_delta:
                ...
                torch.save({
                    ...
                    "in_channels": 3,
                    "model_name": "DenseNet121_3ch_MCI",
                }, ckpt_path)
    ...
    result = {"fold": fold, "best_epoch": best_epoch,
              "best_val_loss": best_val_loss, **best_metrics,
...
def summarize_results(results: list[dict], output_csv: Path) -> None:
    ...
    print("\n=== 5-Fold CV Results (MCI Conversion 3-channel: pMCI vs sMCI) ===")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="5-fold CV — MCI conversion (3-channel T1+GM+WM)"
    )
    ...
    parser.add_argument("--output_csv", type=Path, default=Path("results/cv_mci3ch_results.csv"))
...
def main() -> int:
    ...
    if best_ckpt.exists():
        shutil.copy2(best_ckpt, args.checkpoint_dir / "best_mci3ch.pth")
        print(f"Best fold (AUC={best_fold['auc']:.4f}) → checkpoints/best_mci3ch.pth")
    return 0
