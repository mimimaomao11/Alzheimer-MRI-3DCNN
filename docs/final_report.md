...
| **Exp 10** | **LightCNN3D v2** | **0.744 ± 0.082** | **▲ +0.173** |

#### 最終結果（LightCNN3D MCI v2，Exp 10）
...
|------|----------|-------------|------|
| 門檻值 | 0.500 | **0.758** | — |
| Sensitivity | 0.771 | 0.596 ± 0.128 | −0.175 |
| Specificity | 0.298 | **0.930 ± 0.076** | +0.632 |

聚合混淆矩陣（@Youden 0.758，131 samples）：
...
> **兩種使用情境**：
> - @0.5（高 sensitivity 76%）：適合初步篩查，不漏掉轉換者
> - @Youden 0.758（高 specificity 93%）：適合確認性診斷，FP 極少
...
| MCI 轉換 | LightCNN3D MCI v2 | **0.744 ± 0.082** | 0.758（Youden） | 0.596 | 0.930 |

兩個任務均以 **LightCNN3D（587K params）+ Focal/CE loss + CosineAnnealingLR + Youden's J 門檻校準** 達到最佳效能。若要進一步提升，最優先的方向是擴充 MCI 資料至 150+ subjects（預期 AUC ▲ 0.03–0.06）。
...
| `results/youden_eval_mci.csv` | MCI Youden's J 評估 |
